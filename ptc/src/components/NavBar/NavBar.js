...
export default function NavBar() {
  ...
  const handleLogout = () => {
    logout();
    setTimeout(() => navigate('/login'), 500); // Delay to ensure state updates
  };

  const handleNavigation = (to, openInNewTab = false) => {
    openInNewTab ? window.open(to, '_blank') : navigate(to);
  };

  const userNavigation = [
    { name: 'Your Profile', to: '/profile' },
    ...(user && user.role === 'admin'
      ? [{ name: 'Dashboard', to: '/dashboard', openInNewTab: true }]
            ...
            <div className="flex h-16 items-center justify-between">
              <div className="flex items-center">
                ...
                <div className="hidden md:block ml-10 flex items-baseline space-x-4">
                  <Link to="/earn" className="text-gray-700 hover:text-[#29625d] rounded-md px-3 py-2 text-sm font-medium">Earn</Link>
                  <Link to="/surveys" className="text-gray-700 hover:text-[#29625d] rounded-md px-3 py-2 text-sm font-medium">Surveys</Link>
                  <Link to="/blog" className="text-gray-700 hover:text-[#29625d] rounded-md px-3 py-2 text-sm font-medium">Blog</Link>
                  <Link to="/contact" className="text-gray-700 hover:text-[#29625d] rounded-md px-3 py-2 text-sm font-medium">Contact Us</Link>
                </div>
              </div>
              {isLoggedIn && (
                <div className="ml-4 flex items-center md:ml-6">
                  <button type="button" className="relative rounded-full p-1 text-gray-900 hover:text-[#29625d] focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
                  ...
                  <Menu as="div" className="relative ml-3 z-50">
                    ...
                    <Menu.Items className="absolute right-0 z-50 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                      ...
                      {user && user.membership === 'basic' && (
                        <Menu.Item>
                          <button onClick={() => handleNavigation('/get-started')} className="block px-4 py-2 text-sm font-[500] text-red-500 w-full text-left">Upgrade to Pro</button>
                        </Menu.Item>
                      )}
                      {user && (
                        <Menu.Item>
